...
export const PrivacyPolicyContentLatest: FC<Props> = ({
  specifiedCommercialTransactionActUrl,
  lineAppName,
  companyName,
  companyAddress,
}) => {
  const specialCommercialTransactionLink =
    specifiedCommercialTransactionActUrl != '' ? (
  ...
  return (
    <>
      ...
      <Text pt="20px" className="text-small">
        {specialCommercialTransactionLink}
      ...
      <Text pt="20px" className="bold-medium">
        11 LINEミニアプリに関する追加条項
      </Text>
      <Text className="text-small">
        LINEミニアプリを利用される場合、以下のLINEミニアプリに関する追加条項が適用されます。
      </Text>
      <br />
      <Text className="text-small">
        株式会社Chompyは、{lineAppName}を{companyName}に提供しています。このサービスを提供するにあたり、
        {companyName}の公式なプライバシーポリシーを株式会社Chompyのドメイン内で作成しています。
      </Text>
      <br />
      <Box as="section">
        <Text className="text-small" mb="0">
          【サービス提供者】
        </Text>
        <UnorderedList>
          <ListItem>{companyName}</ListItem>
          <ListItem>{companyAddress}</ListItem>
        </UnorderedList>
      </Box>
      ...
      <Text pt="20px" className="text-small" align="right">
        ２０２１年４月１日制定
        <br />
        ２０２２年１１月８日最終改訂（LINEミニアプリに関する追加条項の追記）
      </Text>
    </>
  );
};
